    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($absensi as $key => $item)
                @php
                    $isLibur = \Carbon\Carbon::parse($item->tanggal)->dayOfWeek === 0;
                    $tanggal = \Carbon\Carbon::parse($item->tanggal);
                @endphp
                <tr class="{{ $isLibur ? 'libur' : '' }}">
                    <td class="text-center">{{ $key + 1 }}</td>
                    <td class="text-center">
                        {{ $tanggal->format('d/m/Y') }}
                        <span class="day-text">{{ $tanggal->format('l') }}</span>
                    </td>
                    <td class="text-left name-cell">{{ $item->user->name }}</td>
                    <td class="text-left">{{ $item->user->divisi }}</td>
                    <td class="text-left">{{ $item->user->jabatan }}</td>
                    <td class="text-center">
